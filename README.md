...
Both remaining errors from the v2 build attempt were the same underlying
cause, unrelated to platform/framework versioning:

`platformio.ini` set `-std=c++17` and explicitly unflagged the framework's
default `-std=gnu++11`. Arduino-ESP32's bundled IDF 4.x headers
(`freertos/portmacro.h`'s `portGET_ARGUMENT_COUNT()`, `hal/gpio_ll.h` and
`hal/misc.h`'s `typeof()`-based register macros) depend on GNU C++
extensions that `gnu++11` provides and that strict `-std=c++17` does not
guarantee. Nothing in this project's own source needs C++17, so the fix is
to simply not override the framework's default standard — removed both the
`-std=c++17` flag and the `build_unflags` line entirely.

The v2 `espressif32@6.9.0` repin was not the actual fix and is left in
place only because it's a valid, working pin on its own merits — it did not
address either error, both of which were coming from the `-std=` override
I had added myself.

Reviewed the full source tree by hand for genuine C++17-only syntax
(structured bindings, `if constexpr`, class template argument deduction,
etc.) before reverting the standard — none present, so this should not
regress under `gnu++11`. Still unverified by an actual compiler in this
sandbox; please run `pio run` again and report back.
